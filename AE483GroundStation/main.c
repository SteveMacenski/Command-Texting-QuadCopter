...
#define OBST_ID					4 //Set below zero if not using Obstacle, if using, set to the actual ID defined in Motive.
...
int main(int argc, char *argv[])
{
	...
	ServerAddress.S_un.S_un_b.s_b4 = 92;
	...
	if ((filename = malloc(strlen("C:\\ae483_yng5\\project\\filename.txt") + strlen(text) + 1)) != NULL){
		filename[0] = '\0';   // ensures the memory is an empty string
		strcat(filename, "C:\\ae483_yng5\\project\\");
	...
	//fputs("ACI Sent:   Flight Time (Sec)	torque_x (N-m)		torque_y (N-m)		torque_z (N-m)		thrust (N)		x (m)		y (m)		z (m)	Theta_x (rad)	Theta_y (rad)	Theta_z (rad) \n", FDR);
	//fputs("ACI Sent:   Flight Time (Sec)	X error(m)		Y error(m)		roll desired(rad)		pitch_desired(rad)		x (m)		y (m)		z (m)	Theta_x (rad)	Theta_y (rad)	Theta_z (rad) \n", FDR);
	fputs("ACI Sent:   Flight Time (Sec)	DeltaT	  CMDs    X speed(m/s)		Y speed(m/s)	x (m)		y (m)		z (m)	Theta_x (rad)	Theta_y (rad)	Theta_z (rad) \n", FDR);
			...
			if (StartREC == 1){
				...
				if (start == 1) {
					...
					begin = clock();
								...
								//fprintf(FDR, "ACI Sent:	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f %0.4f	%0.4f	%0.4f	%0.4f\n", timer, Xerror, Yerror, roll_desired, pitch_desired, QuadCurrent.Pos.x, QuadCurrent.Pos.y, QuadCurrent.Pos.z, QuadCurrent.Ori.tx, QuadCurrent.Ori.ty, QuadCurrent.Ori.tz);
				//fprintf(FDR, "%0.4f	%0.4f	%0.1f    %0.4f	%0.4f	%0.4f %0.4f	%0.4f	%0.4f	%0.4f\n", timer, timeelapsed, numCMDs, Xspeed, Yspeed, QuadCurrent.Pos.x, QuadCurrent.Pos.y, QuadCurrent.Pos.z, QuadCurrent.Ori.tx, QuadCurrent.Ori.ty, QuadCurrent.Ori.tz);
				//printf("Xpos=%0.9f,Xspd=%0.4f,Yspd=%0.4f,CMDs=%.1f,%d\r", QuadCurrent.Pos.x,Xspeed,Yspeed,numCMDs,dani);
				
				fprintf(FDR, "ACI Sent:	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f	%0.4f %0.4f	%0.4f	%0.4f	%0.4f\n", timer, timeelapsed, numCMDs, Xspeed, Yspeed, QuadCurrent.Pos.x, QuadCurrent.Pos.y, QuadCurrent.Pos.z, QuadCurrent.Ori.tx, QuadCurrent.Ori.ty, QuadCurrent.Ori.tz);
				printf("Quad Pos and Obst Pos: %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f \r", QuadCurrent.Pos.x, QuadCurrent.Pos.y, QuadCurrent.Pos.z, ObstCurrent.Pos.x, ObstCurrent.Pos.y, ObstCurrent.Pos.z, QuadDesired.Pos.x, QuadDesired.Pos.y, QuadDesired.Pos.z, QuadDesired.Tz);
			}
			data_updated = 0;
		}
		//Dan Block Commented out
